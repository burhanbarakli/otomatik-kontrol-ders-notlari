...
window.createEscalator=function(root){
  ...
  <section class="esc-loop"><div class="esc-loop-heading"><h2>İşlemci, yükü hızdaki değişimden fark eder</h2><p>Hız sensörü → karşılaştırma → kontrol komutu → motor torku</p></div>
    <svg viewBox="0 0 1200 265" role="img" aria-labelledby="esc-loop-title"><title id="esc-loop-title">Yük bozucusu içeren negatif geri beslemeli hız kontrol döngüsü</title><defs><marker id="esc-loop-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto"><path d="M0 0L10 5L0 10Z" fill="#647f9b"/></marker><marker id="esc-dist-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto"><path d="M0 0L10 5L0 10Z" fill="#bc641c"/></marker></defs>
      <g class="esc-loop-wire" marker-end="url(#esc-loop-arrow)"><path id="esc-wire-ref" d="M135 140H161"/><path id="esc-wire-error" d="M201 140H252"/><path id="esc-wire-drive" d="M439 140H488"/><path id="esc-wire-motor" d="M647 140H696"/><path id="esc-wire-out" d="M948 140H1155"/><path id="esc-wire-sense" d="M1058 140V223H915"/><path id="esc-wire-back" d="M718 223H181V161"/></g>
      ...
      <circle class="esc-sum" cx="181" cy="140" r="20"/><text x="148" y="127" class="esc-sign">+</text><text x="158" y="182" class="esc-sign">−</text><text x="225" y="103" class="esc-small">Hata</text><text id="esc-loop-error" x="219" y="194" class="esc-small">0,000</text>
      <g id="esc-processor"><rect x="252" y="97" width="187" height="86" rx="8"/><text x="346" y="124">İşlemci · PI</text><text id="esc-loop-command" x="346" y="150" class="esc-loop-value">40,0 N·m</text><text id="esc-loop-action" x="346" y="172" class="esc-small">Torku koru</text></g>
      <g><rect x="488" y="104" width="159" height="72" rx="8"/><text x="568" y="132">Motor sürücüsü</text><text x="568" y="157" class="esc-small">Komutu uygular</text></g>
      <g><rect x="696" y="104" width="252" height="72" rx="8"/><text x="822" y="132">Motor + merdiven</text><text id="esc-loop-torque" x="822" y="157" class="esc-loop-value">40,0 N·m</text></g>
      <text x="1075" y="111" class="esc-small">Gerçek hız</text><text id="esc-loop-speed" x="1080" y="166" class="esc-loop-value">0,500 m/s</text>
      <g><rect x="718" y="199" width="197" height="48" rx="8"/><text id="esc-loop-measured" x="817" y="229">Sensör · 0,500 m/s</text></g><text x="456" y="250" class="esc-small">Ölçülen hız geri beslenir</text>
      <text x="828" y="28" class="esc-dist-title">BOZUCU · YOLCU YÜKÜ</text><text id="esc-loop-load" x="828" y="51" class="esc-dist-value">Ek yük torku: 0,0 N·m</text><path id="esc-dist-path" d="M828 60V104" fill="none" stroke="#bc641c" stroke-width="2.5" marker-end="url(#esc-dist-arrow)"/><text x="851" y="96" class="esc-dist-value">−</text>
      <g id="esc-loop-dots">${['ref','error','drive','motor','out','sense','back'].map(key=>`<circle id="esc-dot-${key}" r="4" fill="#2950db"/>`).join('')}</g>
    </svg>
  </section>
  ...
  function values(){
    ...
    $('esc-loop-error').textContent=fmt(error).replace('-','−')+' m/s';$('esc-loop-command').textContent=fmt(s.command,1)+' N·m';$('esc-loop-torque').textContent=fmt(s.motor,1)+' N·m';$('esc-loop-speed').textContent=fmt(s.speed)+' m/s';$('esc-loop-measured').textContent='Sensör · '+fmt(s.measured)+' m/s';$('esc-loop-load').textContent='Ek yük torku: '+fmt(s.load,1)+' N·m';
  ...
  function scene(){
    ...
    if(!reduced.matches&&s.closed)for(const [i,key]of ['ref','error','drive','motor','out','sense','back'].entries()){const p=$('esc-wire-'+key),point=p.getPointAtLength(((s.time/2.2+i*.13)%1)*p.getTotalLength());$('esc-dot-'+key).setAttribute('cx',point.x);$('esc-dot-'+key).setAttribute('cy',point.y);}
  }
